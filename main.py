...
def compile_ui_files():
    ...
    try:
        ...
        logging.info("UI 파일 컴파일 중...")
        ui_dir = os.path.join('ui', 'ui_files')
        output_dir = os.path.join('ui', 'generated')

        success, fail = compile_all_ui_files(ui_dir, output_dir)

        if fail > 0:
            logging.warning(f"UI 파일 컴파일 결과: 성공 {success}개, 실패 {fail}개")
        else:
            logging.info(f"UI 파일 컴파일 완료: {success}개 파일")

        return success > 0  # 하나 이상 성공했으면 True 반환

    except ImportError as e:
        logging.error(f"UI 컴파일러를 가져올 수 없습니다: {str(e)}")
        return False
    except Exception as e:
        logging.error(f"UI 파일 컴파일 중 오류 발생: {str(e)}")
        return False
...
def show_splash_screen(app):
    """스플래시 화면 표시"""
    # 스플래시 이미지 경로
    splash_image_path = os.path.join('resources', 'images', 'splash.png')
