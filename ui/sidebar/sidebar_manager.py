...
import os
from PyQt5.QtWidgets import QWidget, QVBoxLayout, QHBoxLayout, QFrame, QObject
...
class SidebarManager(QObject):
    ...
    def setup_sidebar(self, sidebar_frame, camera_manager=None, detector_manager=None):
        ...
        # YOLO 설정 그룹 초기 숨김
        if hasattr(self.sidebar_widget, 'yoloGroup'):
    ...
    def show_page(self, index):
        ...
        # 스택 위젯 페이지 변경
        if hasattr(sidebar, 'stackedWidget'):
    ...
    def toggle_sidebar(self):
        ...
        if self.sidebar_expanded:
            # 축소
            if hasattr(sidebar, 'contentWidget'):
    ...
    def refresh_cameras(self):
        ...
        # 현재 선택된 카메라 저장
        current_camera = ""
        if hasattr(sidebar, 'cameraCombo'):
        ...
        # 해상도 콤보박스 업데이트
        if hasattr(sidebar, 'resolutionCombo') and sidebar.resolutionCombo.count() == 0:
    ...
    def connect_camera(self):
        ...
        camera = sidebar.cameraCombo.currentText() if hasattr(sidebar, 'cameraCombo') else ""
        resolution = sidebar.resolutionCombo.currentText() if hasattr(sidebar, 'resolutionCombo') else "640x480"
        fps = sidebar.fpsSpinBox.value() if hasattr(sidebar, 'fpsSpinBox') else 30
    ...
    def on_detection_toggled(self, state):
        ...
        # YOLO 설정 그룹 표시 여부
        if hasattr(sidebar, 'modeCombo') and hasattr(sidebar, 'yoloGroup'):
    ...
    def on_detection_mode_changed(self, index):
        ...
        if not hasattr(sidebar, 'modeCombo') or not hasattr(sidebar, 'yoloGroup'):
            return
        ...
        # YOLO 설정 그룹 표시 여부
        is_yolo = (mode == "YOLO")
        is_enabled = sidebar.detectionCheckBox.isChecked() if hasattr(sidebar, 'detectionCheckBox') else False
        sidebar.yoloGroup.setVisible(is_yolo and is_enabled)
    ...
    def on_yolo_version_changed(self, index):
        ...
        if not hasattr(sidebar, 'versionCombo') or not hasattr(sidebar, 'modelCombo'):
            return
    ...
    def on_confidence_changed(self, value):
        ...
        if not hasattr(sidebar, 'confidenceValueLabel'):
            return
    ...
    def apply_detector_settings(self):
        ...
        if not hasattr(sidebar, 'modeCombo'):
            return
        ...
        if detection_mode == "YOLO":
            if not hasattr(sidebar, 'versionCombo') or not hasattr(sidebar, 'modelCombo') or not hasattr(sidebar,
                                                                                                         'confidenceSlider'):
                return
    ...
    def update_detector_info(self, info):
        ...
        if not hasattr(sidebar, 'currentSettingsLabel'):
            return
